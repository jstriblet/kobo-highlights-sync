...
def get_book_highlights(book_id):
    """Fetch highlights for a book from Calibre's annotations table."""
    import sqlite3 as _sqlite3
    import json as _json
    try:
        library_dir = calibre_db.config_calibre_dir
        if not library_dir:
            return []
...
def show_book(book_id):
    ...
    if entries:
        ...
        entry.kobo_progress = None
        entry.kobo_reading_minutes = None
        ...
        if kobo_state:
            if kobo_state.current_bookmark:
                entry.kobo_progress = kobo_state.current_bookmark.progress_percent
            if kobo_state.statistics:
                entry.kobo_reading_minutes = kobo_state.statistics.spent_reading_minutes
